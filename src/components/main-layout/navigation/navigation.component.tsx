    ...
    navigationItems.map(({ to, text }) => {
      return (
        <AnchorLink key={to} href={`#${to}`} offset={isSmall ? '56px' : '64px'} className="all_unset">
          <Button color="inherit" size="large" fullWidth={isSmall} onClick={onCloseHandler} style={{ color: 'white' }}>
            {text}
          </Button>
        </AnchorLink>
      );
    ...
    <>
      ...
      <Hidden smUp>
        ...
        >
          ...
          <Box display="flex" flexDirection="column" py={3} width="100%" height="100vh" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
            {mappedItems}
          </Box>
        </Dialog>
      </Hidden>
